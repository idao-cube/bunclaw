...
describe("text encoding hygiene", () => {
  test("source/docs should not contain replacement char", async () => {
    ...
    for await (const file of glob.scan({ cwd: process.cwd(), absolute: false })) {
      if (file.startsWith(".git/") || file.startsWith("node_modules/") || file.startsWith(".bunclaw/") || file.startsWith(".tmp-") || file.startsWith("temp/") || file.endsWith("encoding-hygiene.test.ts")) continue;
